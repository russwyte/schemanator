...
object SchemaEvolution:
  ...
  private def compareRecords(oldRecord: Schema.Record[?], newRecord: Schema.Record[?]): EvolutionResult =
    ...
    val changedFields: List[FieldChange.FieldTypeChanged] =
      oldFields.keySet.intersect(newFields.keySet).toList.flatMap { name =>
  ...
  private def determineCompatibility(
    ...
    if hasBreakingAdditions then messages += "Added required fields break backward compatibility"
    if hasRemovals then messages += "Removed fields break forward compatibility"
    if hasChanges then messages ++= changed.map(c => s"Field '${c.name}' type changed")
